...
func Timeout(timeout time.Duration, errTimeout *apperrors.Error) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		panicChan := make(chan interface{}, 1) // used to handler panics if you cant recover
		...
		go func() {
			...
			c.Next() // calls subsequest middlewares and handlers
			finished <- struct{}{}
		}()

		select {
		case <-panicChan:
			// if we can not recover from panic, send internal server error
